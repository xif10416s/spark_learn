...
import org.fxi.test.spark.core.mock.communication.core.RpcAddress
...
/**
  * 不需要返回的消息, 最终通过TransportClient.send方法发送消息
  * @param content
  */
...
/**
  * rpc调用消息，需要返回
  * 最终调用TransportClient的sendRpc， 发送请求的RequestId会记录下来
  * @param content
  * @param _onFailure
  * @param _onSuccess
  */
...
/**
  * 发送的信箱 ， 包含了用于发送消息的netty客户端TransportClient
  * @param nettyEnv
  * @param address
  */
  ...
  private def launchConnectTask(): Unit = {
    connectFuture = nettyEnv.clientConnectionExecutor.submit(new Callable[Unit] {
      ...
      override def call(): Unit = {
        try {
          val _client = nettyEnv.createClient(address.host,address.port,address.host)
          outbox.synchronized {
            client = _client
            if (stopped) {
              closeClient()
