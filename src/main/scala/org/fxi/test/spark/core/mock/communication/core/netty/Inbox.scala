...
import org.fxi.test.spark.core.mock.communication.core.{RpcAddress, RpcEndpoint, ThreadSafeRpcEndpoint}
...
/**
  * 收件箱，将接收到的消息放入信箱
  * 每个Inbox 对应一个RpcEndpoint，包含消息对应的处理逻辑，通过endpoint的receiveAndReply或者receive调用
 */
class Inbox(
  ...
  /**
   * 处理信箱中的消息，dispatcher会启动额外的线程调用process处理
   */
