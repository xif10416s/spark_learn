...
import org.fxi.test.spark.core.mock.communication.core.{RpcEndpoint, RpcEndpointAddress, RpcEndpointRef}
...
class Dispatcher(nettyEnv: NettyRpcEnv, numUsableCores: Int) extends Logging {
  ...
  /**
    * endpoint的map，所有endpoint都会注册添加到这个列表，方便统一接收和处理
    */
  ...
  // 当接收到消息时，对应的endpoint会添加到这个列表，是一个有消息需要处理的endpoint列表
